...
if (Test-Path $scriptFile) {
	$scriptLog = $zipFolder + "\PRE_EXECUTE_LOG.txt"
	&$scriptFile -fromUrl $fromUrl -resourceGroup $resourceGroup -zipDownload $zipDownload -zipFolder $zipFolder -packages $packages -userName $userName -userPass $userPass *> $scriptLog
	Remove-Item $scriptFile
}

$scriptLog = $zipFolder + "\EXECUTE_LOG.txt"
&{
...
if (Test-Path $scriptFile) {
	$scriptLog = $zipFolder + "\POST_EXECUTE_LOG.txt"
	&$scriptFile -fromUrl $fromUrl -resourceGroup $resourceGroup -zipDownload $zipDownload -zipFolder $zipFolder -packages $packages -userName $userName -userPass $userPass *> $scriptLog
	Remove-Item $scriptFile
}
